...
xyz <- ggplot(ur.viz, aes(x=Date, group = 1)) + 
  geom_line(aes(y = Unemployment_Rate, group = 1), color = "darkred") +
  geom_line(aes(y = percentcases, group = 1), color="steelblue", linetype="twodash") + geom_line(aes(y = percentdeaths, group = 1), color="darkgreen", linetype="twodash") + labs(x = "Time Period", y = "Percentage", caption="Source: Center for Disease Control (CDC) and U.S. Bureau of Economic Analysis (BEA)") + ggtitle("Unemployment Rate Trends in the U.S. 2020-21")  
...
p <- ur.viz %>%
  ggplot(aes(Date, Monthly_Change, size = 1, color="blue")) + 
  geom_point() +
  geom_line(aes(y = percentcases, group = 1), color="steelblue", linetype="twodash") + 
  theme_bw()
